...
export async function POST(req: Request) {
  try {
    rateLimitOrThrow(req, { keyPrefix: 'tool:product_mockup_generator', limit: 20, windowMs: 60_000 })
    ...
    const resp = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent', {
      ...
      body: JSON.stringify({ contents: [{ parts: [{ text: buildPrompt(body) }] }], generationConfig: { temperature: 0.65, topP: 0.95, maxOutputTokens: 1800 } }),
    })
